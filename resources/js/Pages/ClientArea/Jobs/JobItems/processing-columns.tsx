...
import { Category, JobItem, ProcessingDataStatus, ItemStatus } from "./types";
...
export const processingColumns: ColumnDef<JobItem>[] = [
      ...
      return (
        <Select
          ...
          onValueChange={(value) => {
            ...
            // Always use the category's default weight when selecting a category
            if (selectedCategory) {
              row.original.processing_weight = selectedCategory.default_weight || null;
            } else {
              row.original.processing_weight = null;
            }
      ...
      return (
        <Select
          ...
          onValueChange={(value) => {
            ...
            // Override with subcategory's default weight if available
            if (selectedSubCategory?.default_weight) {
              row.original.processing_weight = selectedSubCategory.default_weight;
            }
  ...
  {
    accessorKey: "processing_serial_number",
    ...
    cell: ({ row, table }: CellContext<JobItem, unknown>) => {
      ...
      return isEditable ? (
        <Input
          value={row.original.processing_serial_number || ""}
          onChange={(e) => {
            row.original.processing_serial_number = e.target.value;
            meta?.setItems?.([...table.options.data]);
          }}
          placeholder="Enter serial number"
        />
      ) : (
        <span>{row.original.processing_serial_number || '-'}</span>
      );
    }
  },
  {
    accessorKey: "processing_asset_tag",
    header: "Asset Tag",
    minSize: 180,
    cell: ({ row, table }: CellContext<JobItem, unknown>) => {
      const meta = table.options.meta as CustomTableMeta | undefined;
      ...
      return isEditable ? (
        <Input
          value={row.original.processing_asset_tag || ""}
          onChange={(e) => {
            row.original.processing_asset_tag = e.target.value;
            meta?.setItems?.([...table.options.data]);
          }}
          placeholder="Enter asset tag"
        />
      ) : (
        <span>{row.original.processing_asset_tag || '-'}</span>
      );
    }
  },
  {
    accessorKey: "processing_weight",
    header: "Weight (kg)",
    minSize: 150,
    cell: ({ row, table }: CellContext<JobItem, unknown>) => {
      const meta = table.options.meta as CustomTableMeta | undefined;
      ...
      return isEditable ? (
        <Input
          value={row.original.processing_weight || ""}
          onChange={(e) => {
            row.original.processing_weight = e.target.value;
            meta?.setItems?.([...table.options.data]);
          }}
          placeholder="Enter weight"
        />
      ) : (
        <span>{row.original.processing_weight || '-'}</span>
      );
    }
  },
  {
  ...
  },
  {
    ...
    header: ({ table }) => {
      const meta = table.options.meta as CustomTableMeta | undefined;
      return ['Processing', 'Completed'].includes(meta?.jobStatus || '') ? "Item Status" : null;
    },
    minSize: 200,
    cell: ({ row, table }: CellContext<JobItem, unknown>) => {
      const meta = table.options.meta as CustomTableMeta | undefined;
      ...
      if (!isEditable) {
        return <span>{row.original.item_status || '-'}</span>;
      }

      return (
        <Select
          value={row.original.item_status || ""}
          onValueChange={(value) => {
            row.original.item_status = value as ItemStatus;
            meta?.setItems?.([...table.options.data]);
          }}
        >
          <SelectTrigger>
          ...
          <SelectContent>
            ...
            <SelectItem value="Repurposing Parts">Repurposing Parts</SelectItem>
