...
export default function QuotesEdit({ job, customers, addresses, collection_types, sanitisation_options, status_options, staff_members,sub_clients }: Props) {
  const [isDeleting, setIsDeleting] = useState(false);
  
  // Replace the entire handleDeleteJob function
  const handleDeleteJob = () => {
    setIsDeleting(true);
    router.delete(`/quotes/${job.id}`, {
      onSuccess: () => {
  ...
  const handleSaveChanges = async () => {
      try {
          ...
          const response = await axios.put(`/quotes/${job.id}`, dataToSend);
        ...
        <header className="flex h-16 items-center gap-2 px-4 bg-white border-b">
          ...
          <Breadcrumb>
            <BreadcrumbList>
              ...
              <BreadcrumbSeparator />
              <BreadcrumbItem>
                <BreadcrumbLink href="#">Edit Quote</BreadcrumbLink>
              </BreadcrumbItem>
            </BreadcrumbList>
          </Breadcrumb>
        </header>
