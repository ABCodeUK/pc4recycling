...
export default function JobItems({ jobId, jobStatus, job }: { jobId: string; jobStatus: string; job: any }) {
  ...
  const isEditingAllowed = () => {
    const editableStatuses = ['Quote Requested','Needs Scheduling', 'Request Pending', 'Scheduled', 'Postponed'];
    ...
    <>
      <section className="bg-white border shadow rounded-lg">
        <header className="p-6 flex justify-between items-center">
          <h2 className="text-lg font-semibold">Items to collect</h2>
          {(jobStatus === "Quote Requested" || jobStatus === "Quote Rejected") && (
            <Button onClick={() => setIsQuoteDialogOpen(true)}>
              Provide Quote
            </Button>
          )}
          {(jobStatus === "Quote Provided" || jobStatus === "Quote Declined") && (
            <Button onClick={() => {
              setIsQuoteDialogOpen(true);
              // Pre-fill data will be handled by the QuoteDialog component
            }}>
              Adjust Quote
            </Button>
          )}
