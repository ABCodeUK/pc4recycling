...
export default function JobItems({ jobId, jobStatus, job }: { jobId: string; jobStatus: string; job?: any }) {
  ...
  const isEditingAllowed = () => {
    const editableStatuses = ['Quote Requested','Quote Draft'];
    return editableStatuses.includes(jobStatus);
  };

  
  // Check if the job status allows item management
  const showItemManagementButtons = jobStatus === 'Quote Requested' || jobStatus === 'Quote Draft';
        ...
        >
          ...
          <TabsContent value="processing" className="mt-6">
            <div className="p-6 text-center text-gray-500">
              <p>Processing information will be available after quote acceptance.</p>
            </div>
          </TabsContent>

          <TabsContent value="completed" className="mt-6">
            <div className="p-6 text-center text-gray-500">
              <p>Completion information will be available when the job is finished.</p>
            </div>
          </TabsContent>
        </Tabs>
      </div>
